...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history']))
    {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query))
        {
            ...
            $str.='<dl><dt><a title="'.$goods['goods_name'].'" target="_blank" href="'.$goods['url'].'"><img src="'.$goods['goods_thumb'].'" title="'.$goods['goods_name'].'"></a></dt><dd><p><a title="'.$goods['goods_name'].'" target="_blank" href="'.$goods['url'].'">'.$goods['goods_name'].'</a></p><span>￥'.$goods['shop_price'].'</span></dd></dl>';
        }
...
function insert_sf_history()
{
    ...
    $output = $GLOBALS['smarty']->fetch('library/sf_history_list.lbi');
    $GLOBALS['smarty']->caching = $need_cache;
...
function insert_cart_info()
{
	...
	$str = '<s class="setCart"></s><a rel="nofollow" class="btn-shopcart" href="flow.php">我的购物车<b id="in_cart_num">'.$number.'</b><span class="outline"></span><div id="cart_lists"><div class="floatcar"><ul id="tEvent">';
	...
	{
		$str .= ' <li>
		  <div class="l"><a target="_blank" href="goods.php?id='.$row['goods_id'].'">
		  <img width="40" height="40" alt="'.$row['goods_name'].'" src="'.$row['goods_thumb'].'"></a></div>
		  <div class="c">'.'<a target="_blank" href="goods.php?id='.$row['goods_id'].'">'.$row['goods_name'].'</a>'.'</div>
		  <div class="r"><font>¥ '.$row['goods_price'].'</font> × '.$row['goods_number'].'<br /><a title="'.$row['goods_name'].'" href="javascript:;" onclick="deleteCartGoods('.$row['rec_id'].')">删除</a></div>
		  </li>';
	}
	
  
   
    $str .= '</ul><div class="total">
			<p style="width:210px;">共有<font id="totalNumber">'.$number.'</font>件商品</p>
            <p style="width:120px;text-align:right;">共计：<font> ¥'.$amount.'</font><br>
				<a class="submit5" href="./flow.php">去购物车结算</a></p>
      	 </div><div class="clear"></div></div>';
	}
	else
	{
		$str = '<a rel="nofollow" class="btn-shopcart" href="flow.php">购物车</a><span class="txt-cartcount" style="display: none;">0</span><div class="shopcart-list"><div class="empty">您的购物车内暂时没有商品，去<a href="index.php">首页</a>挑选</div></div></div>';
	}
    return $str;
}
 ...
 * @return  string
 */
function insert_cart_flow_content2()
{
    $sql = 'SELECT c.*,g.goods_name,g.goods_thumb,g.goods_id,c.goods_number,c.goods_price' .
           ' FROM ' . $GLOBALS['ecs']->table('cart') ." AS c ".
					 " LEFT JOIN ".$GLOBALS['ecs']->table('goods')." AS g ON g.goods_id=c.goods_id ".
    ...
    $output = $GLOBALS['smarty']->fetch('library/cart_flow_content2.lbi');
    return $output;
}
