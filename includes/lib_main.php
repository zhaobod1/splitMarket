...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    $ur_here    ='<strong>' . '<a href=".">' . $GLOBALS['_LANG']['home'] . '</a>' . '</strong>';
    ...
    if ($filename != 'index')
    {
        ...
        if (in_array($filename, array('category', 'goods', 'article_cat', 'article', 'brand')))
        {
            ...
            if (!empty($cat_arr))
            {
                ...
                foreach ($cat_arr AS $val)
                {
                    ...
                    $ur_here   .='<span>' . '>' . ' <a href="' . build_uri($type, $args, $val['cat_name']) . '">' .
        ...
        else
        {
            ...
            if ('group_buy' == $filename)
            {
                ...
                $args       = array('gbid' => '0');
                $ur_here   .='<span>' . '>' . ' <a href="group_buy.php">' .
                                $GLOBALS['_LANG']['group_buy_goods'] . '</a>' . '</span>';
            }
            /* 拍卖 */
...
function assign_template($ctype = '', $catlist = array())
{
	...
	$smarty->assign('tongtai_article', index_get_articles(21,5));    //动态
