...
define('DISCUZ_ROOT', '../');
define('ROOT_PATH', str_replace('api/uc.php', '', str_replace('\\', '/', __FILE__)));
...
class uc_note
{
	...
	function synlogin($get, $post)
	{
		...
		writeLog("********synlogin*********");
		set_login($uid, $username, $this->db);
		writeLog("********synlogin*********");
	...
	function updatecredit($get, $post)
	{
		...
		$db = $GLOBALS['db2'] = uc_api_mysql("user","ReturnDb");

		$credit = $get['credit'];
		$amount = $get['amount'];
		$uid = $get['uid'];
		hasTitleLog("updatecredit 更新用户积分", $get, "w+");
		$sql = "SELECT * FROM `centre_config` WHERE `code`='points_rule'";
		hasTitleLog("updatecredit db:", $GLOBALS['db']);
		$sRes = $db->fetch_first($sql);
		...
		$aUser = uc_get_user($uid, 1);

		hasTitleLog("updatecredit aUser", $aUser[1]);
		$sql = "UPDATE `member` SET `shop_coin`='$amount' WHERE `nickname` = '{$aUser[1]}'";
		$db->query($sql);
		writeLog($sql);
		/*
	    $sql = "UPDATE `member` SET `shop_coin`='$amount' WHERE `nickname` = $aUser[1]";
		...
		foreach($get['credit'] as $appid => $credititems)
		{
			if($appid == UC_APPID)
			{
				foreach($credititems as $value)
				{
					$outextcredits[] = array
					(
						'appiddesc' => $value['appiddesc'],//目标应用程序ID=1
						'creditdesc' => $value['creditdesc'], //积分编号=1
						'creditsrc' => $value['creditsrc'], //源积分编号=0
						'title' => $value['title'], //积分名称="消费积分"
						'unit' => $value['unit'], //积分单位
						'ratio' => $value['ratio'] //比率=1
					);
				}
			}
		}
		hasTitleLog("updatecreditsettings 将积分参数保存到数据库:", $outextcredits);


		include_once "../include/config.inc.php";
		include_once   "../uc_client/client.php";
		$sql = "UPDATE `centre_config`  SET `value`='".uc_serialize($outextcredits)."' WHERE `code`='points_rule'";
		$res = $GLOBALS['db']->query($sql);
		writeLog($sql);
		writeLog($GLOBALS['db']->error());
		return API_RETURN_SUCCEED;
	}
}
 ...
 * @return void
 */
function set_login($user_id = '', $user_name = '', $db = '')
{

	if (empty($user_id)) {
		return;
	} else {
		//writeLog("system --- before");
		$sql="SELECT xtkg FROM `systemparameters` where id=1";
		$row=$db->fetch_first($sql);
		if ($row){
			writeLog("set_login 设置用户登录api --> systemstatus---open");
			$sql = "select * from `Member` where nickname='" . $user_name . "'";
			$memberInfo = $db->fetch_first($sql);
			if ($memberInfo) {
				$us = getMemberbyNickName($user_name);
				$_SESSION['ID'] = $us['id'];
				...
				// 没有用户，不准登录
				writeLog("set_login 设置用户登录api --> log forbidden");
				return false;
