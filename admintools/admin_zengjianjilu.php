...
$cheuid_arr = $_POST['UID'];
	foreach ((array)$cheuid_arr as $id)
	{
    	edit_delete_cl('zengjianjilu',$id);
	}
...
<table width="100%" cellpadding="3" cellspacing="0" border="0" align="center" class="table1">
      ...
      <tr >
        <td colspan="7" align="center">服 务 中 心 增 减 记 录</td>
      </tr>
      <tr>
      ...
      <?php
	  	$pagesize = 20; //设置每页记录数 
	  	$sql = "SELECT * FROM `zengjianjilu` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']."";
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `zengjianjilu` WHERE 1=1 ".$_SESSION['Search']." ".$_SESSION['SearchTime']." order by cdate desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
