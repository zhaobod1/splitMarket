...
<table width="50%" cellpadding="3" cellspacing="1" border="0" align="left" class="table1">
  ...
  <?php
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `stockbuy` WHERE lx=1 and isjs=0 order by price limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
		while ($row=mysql_fetch_array($query)){
	  ?>
  <tr>
...
<table width="50%" cellpadding="3" cellspacing="1" border="0" align="right" class="table1">
  ...
  <?php
		...
		$start = $pagesize * ($p - 1); //计算起始记录 
      	$sql = "SELECT * FROM `stockbuy` WHERE lx=0 and isjs=0 order by price desc limit ".$start.",".$pagesize;
		if($query = mysql_query($sql)){
		while ($row=mysql_fetch_array($query)){
	  ?>
  <tr>
