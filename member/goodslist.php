...
if($_POST['button3']){
	...
	if ($member['ispay']==0){
		...
		if($_SESSION['shoppingcart']<>""){
			if($member['lsk']<>$gwb){
				alert("购物金额错误,请选择价值".$member['lsk']."的产品.","?");
			}else{
				if($member['zsq']>=$gwb){
...
<table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
      ...
      <tr>
        ...
        <td align="center" ><input name="<?=$row['id']?>num" id="<?=$row['id']?>num" type="text" value="0" size="4" onKeyUp="jiage(this,<?=$row['id']?>);"><input name="<?=$row['id']?>" id="<?=$row['id']?>" type="hidden" value="<?=$row['price']?>"></td>
