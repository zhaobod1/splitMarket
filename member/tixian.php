...
if ($_POST['submit']) {
    ...
    //接收提交的信息
	$uid = $_SESSION['ID']; // userId
	$jine = $_POST['jine'];// withdraw deposit
    $withdrawSource = trim($_POST["txly"]);
    if ($withdrawSource == "游戏币") {
        if ($jine <= $yxb)
        {
	        $tixian['uid'] = $uid;
	        ...
	        if ($_POST["txlx"] == "微信支付") {
		        $tixian['bankname'] = "微信支付";
		        $tixian['bankcard'] = $_POST['caifutong'];
	        }
	        if ($_POST["txlx"] == "农业银行") {
		        $tixian['bankname'] = "农业银行";
		        $tixian['bankcard'] = $_POST['BankCard'];
	        }
	        if ($_POST["txlx"] == "建设银行") {
		        $tixian['bankname'] = "建设银行";
		        $tixian['bankcard'] = $_POST['BankCard'];
	        }
	        if ($_POST["txlx"] == "建设银行") {
		        $tixian['bankname'] = "工商银行";
		        $tixian['bankcard'] = $_POST['BankCard'];
	        }
	        if ($_POST["txlx"] == "中国银行") {
		        $tixian['bankname'] = "中国银行";
		        $tixian['bankcard'] = $_POST['BankCard'];
	        }
	        $tixian['bdid'] = $bdid;
	        ...
	        $tixian['tdate'] = now();
	        $tixian['source'] = "游戏币提现 " . time();
	        //$member['mey'] = $mey - $jine;
	        $member['yxb'] = $yxb - $jine;
	        add_insert_cl('tixian', $tixian);
	        edit_update_cl('member', $member, $uid);
	        require_once "../include/lib_h15_func.php";
	        sendSmsToAdminAndClient($us["usertel"],$tixian["source"]);
	        ...
	        echo "<script language=javascript>alert('您的游戏币余额不足,申请提交失败');window.location.href='?'</script>";

        }
    }
	if ($withdrawSource == "奖金") {
		if ($jine <= $mey) {
			$tixian['uid'] = $uid;
			$tixian['nickname'] = $_SESSION['nickname'];
			$tixian['username'] = $us['username'];
...
<form name="form1" method="post" action="?" onSubmit="return CheckForm();">
    <table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
        ...
        <tr>
            <td width="46%" height="22" align="right">游戏币余额：</td>
            <td width="54%" align="left"><?= $yxb ?></td>
        </tr>
        <tr>
            <td width="46%" height="22" align="right">提现来源
                ：
            </td>
            <td width="54%" align="left">
                <select name="txly" id="txly"> <!--提现账户-->
        ...
        <tr>
            ...
            <td width="54%" align="left">
                ...
                <select name="txlx" id="txlx"> <!--提现账户-->
                    <option value="支付宝">支付宝</option>
                    <option value="微信支付">微信支付</option>
                    <option value="农业银行" <?= $BankName=="农业银行" ? "selected=\"selected\"" : "" ?>>农业银行</option>
                    <option value="建设银行" <?= $BankName=="建设银行" ? "selected=\"selected\"" : "" ?>>建设银行</option>
                    <option value="工商银行" <?= $BankName=="工商银行" ? "selected=\"selected\"" : "" ?>>工商银行</option>
                    <option value="中国银行" <?= $BankName=="中国银行" ? "selected=\"selected\"" : "" ?>>中国银行</option>

                </select>
            </td>
        </tr>
        <!-- 青岛火一五信息科技有限公司huo15.com 银行信息 日期：2017/3/14 -->
        <tr>
