					...
					if ($query = mysql_query($sql)) {
						while ($row = mysql_fetch_array($query) and $num > 0) {
							if ($row['num'] <= $num) {
								...
								//扣除手续费
								//获取系统设置的手续费百分比
								$yxb = $yxb * (100 - floatval($sys["txsl"])) / 100;
						...
						if ($query = mysql_query($sql)) {
							while ($row = mysql_fetch_array($query) and $num > 0) {
								if ($row['num'] > $num) {//买的数量大于卖出的数量
									...
									$stockrecord['sdate'] = now();
									$stockrecord['lx'] = $lx;
									add_insert_cl('stockrecord', $stockrecord);//交易记录

									$row['num'] = $row['num'] - $num;
									edit_update_cl('stockbuy', $row, $row['id']);//委托记录
									...
									edit_sql("update `member` set yxb=yxb+" . $yxb . " where id=" . $us['id'] . "");
...
<form name="form1" method="post" action="?" onSubmit="return CheckForm();">
    ...
    <table width="100%" border="0">
        <tr>
            <td valign="top">
                <table width="100%" cellpadding="3" cellspacing="1" border="0" align="left" class="table1">
                    <tr>
            ...
            <td valign="top">
                <table width="100%" cellpadding="3" cellspacing="1" border="0" align="right" class="table1">
                    <tr>
            ...
            <td colspan="2">
                <table width="100%" cellpadding="3" cellspacing="1" border="0" align="center" class="table1">
                    ...
                    <tr>
                        ...
                        <td colspan="2" align="left"><?php echo $sys["peprice"] ?><input name="jine"
                                                                                         value="<?php echo $sys["peprice"] ?>"
                                                                                         type="hidden" id="jine"
                                                                                         size="10" maxlength="20"></td>
                    </tr>
                    <tr>
                        <td height="22" align="right"><?= $weituo10 ?>：</td>
                        <td colspan="2" align="left"><input name="num" type="text" id="num" size="10" maxlength="20">
                        </td>
                    </tr>
                </table>
            </td>
                            ...
                            <tr>
                                ...
                                <td align="center"><?php if ($row['isjs'] == 1) { ?>
									...
									<?php } else if ($row['isjs'] == 0) { ?>
                                        <font color="#FF0000"><a href="?action=Revoke&id=<?= $row['id'] ?>"
                                                                 onClick="{if(confirm('您确定要撤消您的委托吗?')){return true;}return false;}">撤消委托</a></font>
